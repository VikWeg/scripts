// **** Map Thread to Vox ****
__device__ int threadIdToVoxNum(int threadId, int latticeId, Constant Const)
{
	int c0 = Const.c0;
	int c1 = Const.c1;
...
__host__ __device__ int getBit(int i, unsigned long long c)
{
...
__device__ unsigned long long changeBit(int i, unsigned long long c)
{
	unsigned long long mask = 1ull << i;
...
__host__ __device__ int sumBits(unsigned long long c)
{
...
__host__ __device__ int GetVoxNumFromNeighborNum(int VoxNum, int NeighborNum, int* VoxIds, Constant Const)
...
__host__ __device__ int GetNonEmptyNeighborVoxsCount(int BaseVoxNum, int* VoxIds, Constant Const)
...
__host__ __device__ int GetNeighborNumFromVoxNum(int BaseVoxNum, int VoxNum, int* VoxIds, Constant Const)
...
__host__ __device__ int GetConnectivityOffset(int VoxNum, int NeighborNum, int* VoxIds, Constant Const)
		...
		for (j = 0; j < 3; j++)
			...
			if (
				...
				)
				{
					...
					else Offset += Const.SpinCount - VoxIds[vox];
...
__host__ __device__ int GetNeighborSpinCount(int VoxNum, int* VoxIds, Constant Const)
{//Returns number of spins around VoxNum
	...
	if (
		...
		)
		{
			...
			else count += Const.SpinCount - VoxIds[vox];
		}

	return count;
}

__host__ __device__ Next GetNextSpin(int* VoxIds, int VoxNum, int NextSpinOffset, Constant Const)
{//Returns the voxnum and spin id of the spin with spin offset NextSpinOffset relative to VoxNum
	...
	if (
		...
		)
		{
			...
			else SpinsInVoxel = Const.SpinCount - VoxIds[voxnum];
...
__device__ int getMaxThreadId(int LatticeId, Constant Const)
{
	int c0 = Const.c0;
	int c1 = Const.c1;
	int c2 = Const.c2;
