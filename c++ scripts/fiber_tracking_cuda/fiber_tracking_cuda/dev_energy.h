__global__ float Edata(vertex* vi, vertex* vj)
{
	float E1, E2;
	float xij[3];
...
__global__ float Eint(vertex* vj, vertex* vi, vertex* vk)
{
	float xij[3];
	xij[0] = vj->x - vi->x;
	xij[1] = vj->y - vi->y;
...
__global__ float Ei_x(vertex* vi)
{
	int count = 0;
	float E = 0;
...
__global__ float Ei_c(vertex* vi)
{
